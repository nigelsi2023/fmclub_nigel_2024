...
@section('meta_desc', "Read and watch the latest analysis on forex, stock market and commodities from our experienced
Traders")
            ...
            <div class="post border-bottom-style-1 bottom-margin-sm bottom-padding-sm">
                <div class="post-title bottom-line bottom-margin-vsm">
                    ...
                    <h4 class="font-weight-lbold text-capitalize"><a class="text-color-hover"
                            href="{{ route('fulltraderpost', $blog->id) }}">{{ $blog->name }}</a></h4>
                </div>
                <!-- META -->
                <ul class="meta-list">
                    <li><i class="fa fa-user text-color ileft"></i> @if($blog->user->role == 'admin') Admin @else {{
                        $blog->user->name }} @endif</li>
                    ...
                    <li>
                        <ul class="zozo-social-share-icons share-box typo-darks no-left-padding">
                            ...
                            <li class="facebook">
                                <a onclick="window.open('https://www.facebook.com/sharer/sharer.php?u={{ route('fulltraderpost', $blog->id) }}', '_blank', 'location=yes,height=570,width=520,scrollbars=yes,status=yes');"
                            ...
                            <li class="twitter">
                                <a onclick="window.open('https://twitter.com/intent/tweet?text={{ route('fulltraderpost', $blog->id) }}', '_blank', 'location=yes,height=570,width=520,scrollbars=yes,status=yes');"
                                    title="twitter">
                            ...
                            <li class="linkedin">
                                <a
                                    onclick="window.open('https://www.linkedin.com/shareArticle?mini=true&url={{ route('fulltraderpost', $blog->id) }}', '_blank', 'location=yes,height=570,width=520,scrollbars=yes,status=yes');">
                ...
                <div class="row">
                    ...
                    <div class="post-image">
                        @if (empty($blog->image))
                        <iframe width="100%" height="400" src="{{ $blog->videoLink }}">
                        </iframe>
                        @else
                        <a href="{{ route('fulltraderpost', $blog->id) }}">
                            <img class="img-responsive" src="{{ asset('uploads/traders/'.$blog->image) }}" width="1300"
                                height="800" alt="" />
                        </a>
                        @endif
                        <div class="post-date-wrap">
                            <h6 class="post-date">{{ date("M", strtotime($blog->created_at)) }}<span>{{ date("d",
                                    strtotime($blog->created_at)) }}</span></h6>
                        </div>
                    </div>
                    ...
                    <p>{!! $blog->detail !!}</p>
                    @endif
                    <a href="{{ route('fulltraderpost', $blog->id) }}" class="btn typo-light btn-hv-dark theme-bg">Read
                        More</a>
                    <!-- </div> -->
                </div>
            </div>
